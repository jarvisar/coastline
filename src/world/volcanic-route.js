...
// Column indices, counted out from the centre line, of the near and far cliff
// crests. The chunk builder shades by them.
...
// Lava courses flank a driveable shelf. Everything samples world coordinates so
// chunk seams match. The short-span terrace breaks up each cliff line.
function riftShape(s, side) {
  if (side > 0) {
    const { u } = creekLayout(s);
    // On this side the bounds are the creek's supporting shelf.
...
// Two benches with steep scarps. Their breakpoints must also be mesh columns,
// or the extra height just renders as hills.
...
function bankSteps(s, side) {
  // Separate seeds per step so the benches don't form parallel bands.
  const lower = 16 + 16 * terrace(s + 40, 7171 + side, 80);
  const upper = 52 + 24 * terrace(s - 24, 7181 + side, 96);
  return { lower, upper };
}

// Shared by the creek and its shelf so both follow the same grade. Long
// wavelengths keep height changes gradual.
...
function hillsideHeight(s, u) {
  ...
  // Saddles where the tributaries cross let them descend gradually.
  let saddle = 0;
...
// Cliff profile: bevelled lip, steep face, toe under the lava.
// `t` is metres out from the cliff's upper edge.
...
// Spillways across the right terraces. Dressing and collision share these
// paths. Their toes stop above the roadside apron.
...
// One cross-valley river every four chunks. The bridge belongs entirely to
// its middle chunk.
...
export function crossingChannel(crossing, u) {
  ...
  // Three cascades. The 0.12 linear term keeps some grade on the benches.
  ...
  // Outlet sits just above the basin level so the two lava sheets never
  // z-fight at the mouth.
...
export function volcanicNaturalTerrainHeight(s, u) {
  ...
  // Floor, steep bank, then a softer shoulder. The cut fades out before the
  // source so it doesn't lower the creek shelf.
...
export function volcanicHeight(s, u) {
  // Driving samples the deck while the mesh continues through the channel
  // below. Also keeps lane-assisted traffic level.
...
export function volcanicVertex(row, column, jitter = 3) {
  ...
  // Jitter the whole cross-section together. Per-column jitter on tight cliff
  // columns can cross edges and tear the surface. Seeded by row so seams match.
  ...
  // Cliff vertices take their column's height so sideways jitter can't slide
  // a crest vertex down to the lava.
...
export const volcanicDrivingRoute = {
  ...
  // Lava uses the water check to keep tyres out.
  water: (s, u) => {
