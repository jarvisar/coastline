...
// Open-spandrel arch after the Big Sur bridge. Twin ribs carry paired deck
// columns, pylons mark the springings and the approaches stand on column bents.
...
function buildBridge(chunk, bridge) {
  ...
  // Bent to follow the road's curve between the two stations.
  ...
  // Parabola through both springings, level at the crown under the deck slab.
  // Ribs thicken toward their footings.
  ...
  for (const side of [-1, 1]) {
    ...
    if (inChunk(pylon)) {
      ...
      // Footing, two legs and a recessed web.
  ...
  // Only the abutments reach the ground. Elsewhere the edge beam is a shallow fascia.
  function bottom(s, u) {
  ...
  for (let s = first; s < last; s++) {
    ...
    for (const side of [-1, 1]) {
      ...
      // Open balustrades keep the small car visible on the bridge.
      ...
      if (s % 3 === 0) {
        // Posts stop at the rail's underside on its exact facet and grade,
        // so the outer face isn't doubled.
    ...
    // Deck underside keeps the arches solid when seen from the ocean side.
...
function buildPond(chunk, pond) {
  ...
  // Slice the terrain's own faces at the waterline so the water follows the
  // bank exactly and matches the jittered chunk seams.
  ...
  for (let row = firstRow; row < firstRow + CHUNK_LENGTH / TERRAIN_STEP; row++) {
    for (let col = 15; col < terrainColumns(row * TERRAIN_STEP).length - 1; col++) {
      for (const triangle of terrainCell(row, col)) {
        ...
        // The damp margin lies on the terrain planes, its width varying along the shore.
        ...
        for (let i = 1; i < bank.length - 1; i++) for (const p of [bank[0], bank[i + 1], bank[i]]) {
          // Fades to nothing at the dry edge so the margin has no outline.
        ...
        // One colour per terrain facet. Smooth shading gives an airbrushed cyan halo.
